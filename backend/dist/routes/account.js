...
accountRounter.get("/balance", userAuth, async (req, res) => {
...
accountRounter.post("/transfer", userAuth, async (req, res) => {
    ...
    await AccountsModel.updateOne({ user: new mongoose.Types.ObjectId(senderUserId) }, { $inc: { balance: -amount } }).session(session);
    await AccountsModel.updateOne({ user: new mongoose.Types.ObjectId(reciverUserId) }, { $inc: { balance: amount } }).session(session);
